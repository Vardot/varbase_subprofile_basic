...
class ScriptHandler {
  ...
  public static function postDrupalScaffoldProcedure(Event $event) {
    ...
    if ($fs->exists($drupal_root . '/profiles/varbase_subprofile_basic/src/assets/robots-staging.txt')) {
      // Create staging robots file.
      copy($drupal_root . '/profiles/varbase_subprofile_basic/src/assets/robots-staging.txt', $drupal_root . '/robots-staging.txt');
    }

    if ($fs->exists($drupal_root . '/.htaccess')
      && $fs->exists($drupal_root . '/profiles/varbase_subprofile_basic/src/assets/htaccess_extra')) {
      ...
      foreach ($htaccess_lines as $line) {
        ...
        if (strpos($line, "RewriteEngine on") !== FALSE) {
          $lines = array_merge($lines, file($drupal_root . '/profiles/varbase_subprofile_basic/src/assets/htaccess_extra'));
        }
      }
      file_put_contents($htaccess_path, $lines);
    }

    if ($fs->exists($drupal_root . '/profiles/varbase_subprofile_basic/src/assets/development.services.yml')) {
      // Alter development.services.yml to have Varbase's Local development
      // services.
      copy($drupal_root . '/profiles/varbase_subprofile_basic/src/assets/development.services.yml', $drupal_root . '/sites/development.services.yml');
    }
  }
